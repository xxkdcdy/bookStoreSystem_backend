...
public interface BookMapper {
    ...
    // 按时间顺序取最新的8本书
    @Select("select * from book\n" +
            "order by book.date desc limit 16")
    ...
    // 根据书号找到种类
    @Select("select book.cid from book where book.id = #{_parameter}")
    String findCid(String bid);
    ...
    // 添加图书
    @Insert("insert into book(cover,title,author,date,price,newOld,contact,phone,qq,weChat,img_1,img_2,img_3,img_4,img_5,abs,cid,uid)\n" +
